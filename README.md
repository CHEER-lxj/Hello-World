...
引用指向的是值本身而非变量，所以一个引用无法改变另一个引用的指向。
...
c; // [1,2,3,4]
d; // [1,2,3,4]
// c 和 d 仅仅是指向值[1,2,3]，并非持有。所以他们更改的是同一个值。
...
e; // [1,2,3]
f; // [3,4,5]
// f 指向的是数组的引用，而不是 e 的指针，所以不会改变 e 的值。
```

**我们无法决定是值复制还是引用复制,一切由值的类型决定。**

需要通过值复制的方式来传递复合值（如数组），就需要为其创建一个复本。 如 a.slice(...); 相反，要将标量基本类型值传到函数内部并更改，就需要将该值封装到一个复合值（对象，数组）中，然后通过引用复制的方式传递。
